...
const AddMember = () => {
  ...
  const handleAddMember = async () => {
    if (currentUser?.balance < 99) {
      toast.error('You do not have enough balance to add a member')
      return
    }

    // First check if the selected under user has three children or not. If 3 then return
    if (!input.placeUnder) {
    ...
    if (placeUnderUser.children.length >= 3) {
      toast.error('This user already has 3 referrals. Please select another')
      return
    }
    ...
    if (input.referenceId !== 'DR-261211') {
      ...
      const transferMoneyChain = async (referenceId, amounts) => {
        let userToGetMoney = await getUserByReference(referenceId)
        ...
        for (const amount of amounts) {
          ...
          await giveMoneyWhileRegistration(userToGetMoney.myReference, amount)

          // Get the next user in the chain
          userToGetMoney = await getUserByReference(userToGetMoney.referenceId)
        }
      }

      // Usage
      // const amountsToTransfer = [50, 30, 20, 10]
      // await transferMoneyChain(input.referenceId, amountsToTransfer)
    }
    if (success) {
      setIsSpinner(false)
      Swal.fire({
